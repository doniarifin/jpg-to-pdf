...
const ImageCompress = () => {
  ...
  const [results, setResults] = useState<Record<string, { name: string; sizeKB: number }>>({});
  ...
  const activeImages =
    downloadScope === "selected" ? (currentImage ? [currentImage] : []) : images;
  const totalOriginalSize = activeImages.reduce((sum, img) => sum + img.originalSizeKB, 0);
  const estimatedTotalSize = mode === "size" ? activeImages.length * targetSizeKB : undefined;
    ...
    <div className="relative flex-1 min-h-0 overflow-y-auto md:overflow-hidden px-6 md:px-10 py-6 scroll-area">
      {compressing && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 bg-white/80 backdrop-blur-sm">
          ...
          <p className="text-sm font-medium text-gray-700">Compressing images…</p>
        </div>
      )}
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 md:h-full">
        ...
        <div className="relative md:col-span-2 bg-white p-6 rounded-2xl shadow md:flex md:flex-col md:min-h-0">
          <h2 className="text-xl font-semibold mb-4">Compress Images</h2>
                    ...
                    >
                      ...
                      <span
                        className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                          isCurrent ? "border-brand-600" : "border-gray-300"
                        }`}
                      >
                        ...
                        {results[img.id] ? (
                          <p className="text-xs text-gray-500">
                            ...
                            ({Math.max(
                              0,
                              Math.round(
                                (1 - results[img.id].sizeKB / img.originalSizeKB) * 100
                              )
                            )}
                            % smaller)
                          </p>
        ...
        {/* RIGHT: Settings */}
        <SidePanel disabled={isDisabled} overlayText="Please choose images first">
          <h2 className="text-xl font-semibold mb-4">Settings</h2>

          {/* Mode toggle */}
          <div className="mb-6">
            <label className="text-sm text-gray-500 block mb-2">Compression Mode</label>
          ...
          <div className="mb-6 p-3 rounded-lg bg-white text-gray-900">
            <label className="text-sm text-gray-500 block mb-2">Apply compress to</label>
          ...
          {mode === "size" && (
            <div className="mb-6">
              <label className="text-sm text-gray-500 block mb-2">
                Target Size: <span className="font-medium text-gray-900">{targetSizeKB} KB</span>
              </label>
              ...
              <p className="text-xs text-gray-400 mt-2">
                Max is the original size of the selected image ({maxTargetKB} KB) — target can only be smaller.
              </p>
            </div>
          )}
          ...
          {mode === "quality" && (
            <div className="mb-6">
              <label className="text-sm text-gray-500 block mb-2">
                Quality: <span className="font-medium text-gray-900">{Math.round(quality * 100)}%</span>
              </label>
          ...
          {images.length > 0 && (
            <div className="mb-6 p-4 rounded-lg bg-gray-50 border">
              <h4 className="text-sm font-medium text-gray-700 mb-2">Estimated Results</h4>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-gray-500">Original Total</p>
                  <p className="font-semibold text-gray-900">{totalOriginalSize} KB</p>
                </div>
                <div>
                  <p className="text-gray-500">Est. Compressed</p>
                  <p className="font-semibold text-brand-600">
                    {estimatedTotalSize !== undefined ? `~${estimatedTotalSize} KB` : "—"}
                  </p>
                </div>
                <div>
                ...
                <div>
                  <p className="text-gray-500">{mode === "size" ? "Per Image" : "Quality"}</p>
                  <p className="font-semibold text-gray-900">
                    {mode === "size" ? `${targetSizeKB} KB` : `${Math.round(quality * 100)}%`}
                  </p>
                </div>
              </div>
            </div>
